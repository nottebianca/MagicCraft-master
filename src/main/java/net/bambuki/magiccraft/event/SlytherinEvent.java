    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            ...
            for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
                if (player.getInventory().getItem(i).getItem() == ModItems.SLYTHERIN_ARTIFACT.get()) {
                    hasArtifact = true;
                    break;
                }
            }
            ...
            if (hasArtifact) {
                player.addEffect(new MobEffectInstance(MobEffects.ABSORPTION, 220, 0, true, false));
            }
        }
    }
}